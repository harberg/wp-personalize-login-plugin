...
class Personalize_Login_Plugin
{
    ...
    /**
     * A shortchode for rendering the login form.
     *
    ...
    public function render_login_form( $attributes, $content = null ) {
        ...
        if( isset( $_REQUEST['redirect_to'] ) ) {
            $attributes['redirect'] = wp_validate_redirect( $_REQUEST['redirect_to'], $attributes['redirects'] );
        }
        // Error messages
        $errors = array();
    ...
    function redirect_to_custom_login() {
        if( $_SERVER['REQUEST_METHOD'] == 'GET' ) {
            $redirect_to = isset( $REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : null;

            if( is_user_logged_in() ) {
                $this->redirect_looged_in_user( $redirect_to );
                exit;
            }
    ...
    function maybe_redirect_at_authenticate( $user, $username, $password ) {
        ...
        return $user;
    }// end maybe_redirect_at_authenticate
    ...
    private function get_error_message( $error_code ) {
        switch( $error_code ) {
            ...
            case 'invalid_username':
                return __( 'We don\'t have any users with that email address. Maybe you used a different one when signing up?', 'personalize-login' );
            case 'incorrect_password':
                $err = __( 'The password you entered wasn\'t quite right. <a href="%s">Did you forget your password</a>?', 'personalize-login' );
     ...
     */
    public funtion redirect_after_logout() {
        $redirect_url = home_url( 'member-login?logged_out=true' );
        wp_safe_redirect( $redirect_url );
        exit;
    }// end redirect_after_logout

    /**
     * Return the URL to which the user should be redirected after the login
     *
    ...
    public function redirect_after_login( $redirect_to, $requested_redirect_to, $user ) {
        ...
        if( user_can( $user, 'manage_options' ) ) {
            // Use the redirect_to_parameter if on is set, otherwise redirect to admin dashboard.
